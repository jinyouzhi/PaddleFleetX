...
def main():
    ...
    parser.add_argument('-s', '--sequence-length', type=int, default=384, help="sequence length for inference")
    ...
    assert args.device_type in paddle.device.get_all_custom_device_type() + ["gpu", "cpu"], f"device type must in {paddle.device.get_all_custom_device_type()}"
    if args.device_type in paddle.device.get_all_custom_device_type():
        place = paddle.CustomPlace(args.device_type, args.device_id)
    elif args.device_type == "gpu":
        place = paddle.fluid.core.CUDAPlace(args.device_id)
    else:
    ...
    #     prof = profiler.Profiler(targets=[profiler.ProfilerTarget.CPU, profiler.ProfilerTarget.GPU], scheduler=(3, 6))
    #     with prof:
    #         for i in range(6):
    #             start = time.time()
    ...
    #             print(f"[inference][{i+1}/6]: start: {start:.7f} end:{end:.7f} cost:{cost:>13} ms, throughput: {throughput:.5f} sentence/s")
    #             prof.step()
